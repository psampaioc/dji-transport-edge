...
class Evidence:
    ...
    def health(self):
        with self.lock:return {"queue_depth":self.queue.qsize(),"dropped_records":self.dropped,"write_errors":self.errors,"writer_alive":self.worker.is_alive()}
    def close(self):
        self.stop.set(); self.worker.join(timeout=2)
...
class EdgeBridge(Node):
    def __init__(self):
        ...
        defaults={"bind_host":"0.0.0.0","telemetry_port":5500,"frame_metadata_port":5501,"clock_port":5502,"primary_rtp_port":5600,"fpv_rtp_port":5610,"max_json_bytes":1200,"rtp_payload_type":96,"rtp_latency_ms":20,"preview_windows":True,"publish_video":True,"evidence_dir":"evidence","android_clock_host":"","android_clock_port":5502,"clock_ping_interval_s":1.0,"primary_topic":"/dji/primary/image_raw","fpv_topic":"/dji/fpv/image_raw","navigation_topic":"/dji/navigation/state","flight_topic":"/dji/telemetry/flight","rtk_topic":"/dji/telemetry/rtk","gimbal_topic":"/dji/telemetry/gimbal","frame_metadata_topic":"/dji/telemetry/frame_metadata","diagnostics_topic":"/dji/diagnostics","transport_metrics_topic":"/dji/edge/transport_metrics"}
    ...
    def publish_navigation(self):
        snapshot=self.latest_state.snapshot(); flight=snapshot["flight"] or {}; rtk=snapshot["rtk"] or {}; ff=flight.get("data",{}).get("fields",{}); rf=rtk.get("data",{}).get("fields",{}); rlat,rlon=unwrap(rf,"fusion.latitude_deg"),unwrap(rf,"fusion.longitude_deg"); rv=bool(unwrap(rf,"is_being_used",False)) and finite(rlat) and finite(rlon); lat,lon=(rlat,rlon) if rv else (unwrap(ff,"aircraft.latitude_deg"),unwrap(ff,"aircraft.longitude_deg"))
        if not(finite(lat) and finite(lon)): return
        m=NavigationState(); m.header.stamp=self.get_clock().now().to_msg(); m.header.frame_id="wgs84"; m.latitude_deg,m.longitude_deg=float(lat),float(lon); m.altitude_m=float(unwrap(ff,"aircraft.altitude_m",0) or 0); m.heading_deg=float(unwrap(ff,"heading_deg",0) or 0); m.velocity_north_m_s=float(unwrap(ff,"velocity.north_m_s",0) or 0); m.velocity_east_m_s=float(unwrap(ff,"velocity.east_m_s",0) or 0); m.velocity_down_m_s=float(unwrap(ff,"velocity.down_m_s",0) or 0); m.position_source=NavigationState.POSITION_RTK if rv else NavigationState.POSITION_GPS_FALLBACK; m.position_valid,m.rtk_valid=True,rv; m.gps_signal_level=int(unwrap(ff,"gps.signal_level",0) or 0); source=rtk if rv else flight; m.session=snapshot.get("session") or source.get("session",""); m.android_mono_ns=int(source.get("android_mono_ns",0)); m.edge_receive_mono_ns=int(source.get("edge_receive_mono_ns",0)); mapped=source.get("mapped_edge_mono_ns"); m.transport_age_s=float("nan") if mapped is None else (m.edge_receive_mono_ns-mapped)/1e9; self.nav.publish(m)
